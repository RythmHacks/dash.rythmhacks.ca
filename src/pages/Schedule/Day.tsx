...
const Day = (props:any) => {
  return (
    ...
    {props.scheduleData.map((row:ScheduleData[]) => {
      if (row.length === 0) {
        return (
          <div className='h-0 pb-36'></div>
        )
      } else if (row.length === 1) {
        return (
          <div className={`event ${row[0].type}`} style={{minHeight: `${(row[0].height * 144) + ((row[0].height - 1) * 16)}px`, marginTop: `${(row[0].heightOffset * 144) + ((row[0].heightOffset) * 16)}px`}}>
        ...
        return (
          <div className='flex gap-4 h-36'>
            <div className={`event w-1/2 ${row[0].type}`} style={{minHeight: `${(row[0].height * 144) + ((row[0].height - 1) * 16)}px`, marginTop: `${(row[0].heightOffset * 144) + ((row[0].heightOffset) * 16)}px`}}>
              <p className='font-bold text-xl'>{row[0].title}</p>
              <p>{row[0].description}</p>
              <p>{row[0].startTime} {row[0].endTime && <span>- {row[0].endTime}</span>} {row[0].location && <span>@</span>} {row[0].location}</p>
            </div>
